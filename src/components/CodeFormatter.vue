...
const formatText = () => {
  if (!inputText.value.trim()) {
    toast.add({ severity: 'warn', summary: 'Advertencia', detail: 'Por favor, ingrese algún texto', life: 3000 });
    return;
  }

  if (!detectFormat(inputText.value)) {
    toast.add({ severity: 'error', summary: 'Error', detail: 'El texto no es un XML o JSON válido', life: 3000 });
    return;
  }
  ...
  try {
    if (isXml.value) {
      ...
      formattedOutput.value = serializer.serializeToString(xmlDoc)
        .replace(/></g, '>\n<')
        .replace(/(<[^>]+>)/g, (match) => {
    ...
    toast.add({ severity: 'error', summary: 'Error', detail: 'Error al formatear el texto', life: 3000 });
  }
};
...
const copyToClipboard = () => {
  if (formattedOutput.value) {
    ...
    toast.add({ severity: 'success', summary: 'Éxito', detail: 'Texto copiado al portapapeles', life: 3000 });
  }
};
...
<template>
  <div class="card">
    <h2 class="text-2xl font-bold mb-4">Formateador de Código</h2>
    ...
    <div class="flex flex-col gap-4">
      ...
      <div v-if="formattedOutput" class="mt-4">
        ...
        <pre class="bg-gray-100 p-4 rounded-lg overflow-x-auto">{{ formattedOutput }}</pre>
      </div>
    </div>
  </div>
</template>
